...
def train_epoch(args, epoch, model, data_loader, optimizer, scheduler,
    ...
    for iter, data in pbar:
        ...
        # -------- 간단한 로깅용 “가짜 SSIM” -------------------------------
        loss_vals = current_loss.detach().cpu().tolist()      # list[float]
        ssim_vals  = [1.0 - v for v in loss_vals]             # 1-loss ≈ SSIM
        ...
        del output, current_loss, loss, loss_vals, ssim_vals
        torch.cuda.empty_cache()
...
def train(args):
    ...
    train_loss_history = []   # train loss 기록 (augmenter 호환용)
    ...
    if getattr(args, 'resume_checkpoint', None):
        ...
        print(f"[Resume] Loaded '{args.resume_checkpoint}' → epoch {start_epoch}, best_train_loss={best_train_loss:.4f}")
        ...
        if augmenter:
            ...
            augmenter.val_loss_history.extend(train_loss_history)
            print(f"[Resume] Augmenter train_loss_history  복원 완료 ({len(train_loss_history)}개 항목)")
        if mask_augmenter:
            mask_augmenter.val_hist.clear()
            mask_augmenter.val_hist.extend(train_loss_history)
            print(f"[Resume] MaskAugmenter val_loss history 복원 완료 ({len(train_loss_history)}개 항목)")
    ...
    # leaderboard completely disabled in final version
    lb_enable = False
    ...
    train_loss_log_file = os.path.join(args.val_loss_dir, "train_loss_log.npy")
    if getattr(args, 'resume_checkpoint', None) and os.path.exists(train_loss_log_file):
        train_loss_log = np.load(train_loss_log_file)
        print(f"[Resume] 기존 train_loss_log 불러옴, shape={train_loss_log.shape}")
    else:
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if augmenter is not None:
            last_train_loss = train_loss_history[-1] if train_loss_history else None
            augmenter.update_state(current_epoch=epoch, val_loss=last_train_loss)
        # ---- MaskAugmenter 스케줄 업데이트 ----
        if mask_augmenter is not None:
            last_train_loss = train_loss_history[-1] if train_loss_history else None
            mask_augmenter.update_state(current_epoch=epoch, val_loss=last_train_loss)
        ...
        # ② val 데이터도 학습에 포함
        extra_loader = create_data_loaders(
        ...
        # ③ 두 Dataset을 Concat 해서 하나의 DataLoader 구성
        collate_fn = tr_loader.collate_fn
        concat_ds = ConcatDataset([tr_loader.dataset, extra_loader.dataset])
        del tr_loader, extra_loader          # worker 종료
        ...
        # 검증을 수행하지 않으므로 val → train 지표로 대체                                                                             loss_type, ssim_metric)
        ...
        file_path = os.path.join(args.val_loss_dir, "train_loss_log.npy")
        ...
        if scheduler is not None:
            ...
            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                scheduler.step(train_loss)      # 여기서는 “낮을수록 좋음” metric
            else:
                scheduler.step()
