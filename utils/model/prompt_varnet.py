...
class NormUnet(nn.Module):
    def __init__(
        ...
        mask_embed_dim: int = 0,
    ):
        super().__init__()
        self.prompt_embed_dim = prompt_embed_dim
        self.mask_embed_dim = mask_embed_dim
        total_in_chans = in_chans + prompt_embed_dim + mask_embed_dim
        self.unet = Unet(in_chans=total_in_chans, out_chans=out_chans, chans=chans, num_pool_layers=num_pools, drop_prob=drop_prob)
    ...
    def forward(self, x: torch.Tensor, prompt: torch.Tensor, mask_prompt: torch.Tensor) -> torch.Tensor:
        ...
        if self.prompt_embed_dim > 0 and prompt is not None:
            prompt = prompt.view(prompt.shape[0], self.prompt_embed_dim, 1, 1).expand(-1, -1, x.shape[2], x.shape[3])
...
class SensitivityModel(nn.Module):
    ...
    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        left, right = torch.argmin(squeezed_mask[:, :cent].flip(1), dim=1), torch.argmin(squeezed_mask[:, cent:], dim=1)
...
class VarNetBlock(nn.Module):
    ...
    def forward(self, current_kspace: torch.Tensor, ref_kspace: torch.Tensor, mask: torch.Tensor, sens_maps: torch.Tensor, prompt: torch.Tensor, mask_prompt: torch.Tensor) -> torch.Tensor:
        zero = torch.zeros(1, 1, 1, 1, 1).to(current_kspace)
        soft_dc = torch.where(mask.bool(), current_kspace - ref_kspace, zero) * self.dc_weight # [FIX] .bool() for warning
        model_term = self.sens_expand(self.model(self.sens_reduce(current_kspace, sens_maps), prompt, mask_prompt), sens_maps)
        return current_kspace - soft_dc - model_term

class PromptVarNet(nn.Module):
    def __init__(self, num_cascades: int = 12, sens_chans: int = 8, sens_pools: int = 4, chans: int = 18, pools: int = 4,
                 use_checkpoint: bool = False, num_domains: int = 4, prompt_embed_dim: int = 128, mask_embed_dim: int = 16):
        super().__init__()
        self.use_checkpoint, self.num_domains, self.prompt_embed_dim, self.mask_embed_dim = use_checkpoint, num_domains, prompt_embed_dim, mask_embed_dim
        self.sens_net = SensitivityModel(sens_chans, sens_pools)
        if self.prompt_embed_dim > 0: self.domain_embedding = nn.Embedding(num_domains, prompt_embed_dim)
        if self.mask_embed_dim > 0: self.mask_embedding_conv = nn.Conv2d(1, mask_embed_dim, kernel_size=3, padding=1, stride=2)
        self.cascades = nn.ModuleList([VarNetBlock(NormUnet(chans, pools, prompt_embed_dim=prompt_embed_dim, mask_embed_dim=mask_embed_dim)) for _ in range(num_cascades)])

    def forward(self, masked_kspace: torch.Tensor, mask: torch.Tensor, domain_indices: torch.Tensor) -> torch.Tensor:
        sens_maps = self.sens_net(masked_kspace, mask)
        kspace_pred = masked_kspace.clone()
        
        prompt = self.domain_embedding(domain_indices) if self.prompt_embed_dim > 0 and domain_indices is not None else None
        
        mask_prompt = None
        if self.mask_embed_dim > 0 and mask is not None:
            # mask shape: (B, 1, 1, W, 1)
            # 1. 실수부만 선택 후 차원 정리 -> (B, 1, W)
            mask_real = mask[..., 0].squeeze(1)
            
            # [FIX] Conv2d에 맞는 4차원 (B, C, H, W) 형태로 변환
            # (B, 1, W) -> (B, 1, 1, W)
            mask_real_4d = mask_real.unsqueeze(1)
            
            mask_prompt = self.mask_embedding_conv(mask_real_4d.float())

        for cascade in self.cascades:
            if self.use_checkpoint:
                kspace_pred = checkpoint(cascade, kspace_pred, masked_kspace, mask, sens_maps, prompt, mask_prompt, use_reentrant=False)
            else:
        ...
        # [FIX] 원본 VarNet과 동일하게, 최종 출력 크기를 384x384로 고정합니다.
        # center_crop 함수가 알아서 padding과 cropping을 처리합니다.
